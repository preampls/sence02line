...
public final class RomUtils {
    ...
    private static ls.example.t.zero2line.util.RomUtils.RomInfo bean = null;
     ...
     * @return the rom's information
     */
    public static ls.example.t.zero2line.util.RomUtils.RomInfo getRomInfo() {
        if (bean != null) return bean;
        bean = new ls.example.t.zero2line.util.RomUtils.RomInfo();
    ...
    private static String getManufacturer() {
        try {
            ...
            if (!android.text.TextUtils.isEmpty(manufacturer)) {
    ...
    private static String getRomVersion(final String propertyName) {
        String ret = "";
        if (!android.text.TextUtils.isEmpty(propertyName)) {
            ret = getSystemProperty(propertyName);
        }
        if (android.text.TextUtils.isEmpty(ret) || ret.equals(UNKNOWN)) {
            try {
        ...
        if (android.text.TextUtils.isEmpty(ret)) {
            return UNKNOWN;
        }
        return ret;
    }
    ...
    private static String getSystemProperty(final String name) {
        ...
        if (!android.text.TextUtils.isEmpty(prop)) return prop;
    ...
    private static String getSystemPropertyByShell(final String propName) {
        ...
        try {
            ...
            input = new java.io.BufferedReader(new java.io.InputStreamReader(p.getInputStream()), 1024);
    ...
    private static String getSystemPropertyByStream(final String key) {
        try {
            ...
            java.io.FileInputStream is = new java.io.FileInputStream(
                    new java.io.File(android.os.Environment.getRootDirectory(), "build.prop")
            );
            prop.load(is);
