...
public final class BitUtils {
    ...
    public static void main(String[] args) {
        ...
        for (byte i = 7; i >= 0; i--) {
            android.util.Log.d("BitUtils", getBitValue(source, i) + "");

        }
        // 将第6位置为1并输出 , 结果为 75 (0100 1011)
        android.util.Log.d("BitUtils", setBitValue(source, 6, (byte) 1) + "");

        // 将第6位取反并输出, 结果应为75(0100 1011)
        android.util.Log.d("BitUtils", reverseBitValue(source, 6) + "");

        // 检查第6位是否为1，结果应为false
        android.util.Log.d("BitUtils", checkBitValue(source, 6) + "");

        // 输出为1的位, 结果应为 0 1 3
        for (byte i = 0; i < 8; i++) {
            if (checkBitValue(source, i)) {
                android.util.Log.d("BitUtils", i + "");
            }
        }
    }
}
